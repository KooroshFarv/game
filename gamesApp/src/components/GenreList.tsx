  ...
  return (
    ...
    <List> 
      ...
      {data?.results.map(genre => (<ListItem key={genre.id} paddingY={'5px'}>
          ...
          />
          <Button whiteSpace={'normal'} textAlign='left' fontStyle={genre.id === selectedGenreId ? 'italic' : 'normal'} fontWeight={genre.id === selectedGenreId ?'bold' : 'normal' } onClick={() => onSelectGenre(genre)} fontSize='lg' variant='link'>{genre.name}</Button>
          </HStack>
          </ListItem> ))}
    </List>
    </>
